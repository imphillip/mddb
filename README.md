...
本阶段暂时暂停 models.dev 与 BaseLLM/NewAPI metadata 的扩展，集中适配 OpenRouter 的 `/api/v1/models`、model endpoint detail、sitemap/page-only 模型发现。完整内部方案见 `.internal/docs/plans/2026-05-16-openrouter-provider-graph-refactor.md`（维护者本地文档，不作为公开发布表面）。
...
在新 schema 中，OpenRouter `/api/v1/models` 的每一行首先被视为：
...
例如 `anthropic/claude-sonnet-4` 是 OpenRouter provider 下的模型 ID；`anthropic` 同时可帮助推断 author/厂牌，但不自动成为 mddb.dev URL 的 provider parent。
