...
- `data/registry/models.json`：模型注册表，记录自研模型 / canonical model identity、基础规格、模态、能力、alias 与来源证据；
- `data/registry/providers/*.json`：供应商注册表，记录每个 provider 提供的模型 offer、API model id、endpoint、价格、参数与来源证据。
...
- `/`：模型广场，从 `data/registry/models.json` 和 `data/registry/providers/*.json` 生成，支持厂牌筛选、搜索、模态筛选、模型 tag 复制和 USD/CNY 价格切换；
...
- `currency`：默认币种；
- `offers[]`：该供应商提供的模型；
  - `model_id`：指向 `data/registry/models.json` 中的 canonical model id；
  - `model`：展示名；
...
- build 阶段读取 `data/registry/models.json` 和 `data/registry/providers/*.json`；
...
```text
data/
  registry/models.json           核心模型注册表
  registry/providers/*.json      核心供应商 / offer 注册表
  schema/*.schema.json           JSON Schema
web/src/
  lib/registry-graph.ts          data/registry JSON -> provider graph adapter
...
- 修正 `data/registry/models.json` 中的 canonical model identity、alias、author、规格或能力；
- 修正 `data/registry/providers/*.json` 中的 offer、api model id、endpoint、价格或来源；
