...
describe('renderUpdateAdminPage', () => {
  it('renders an internal password-gated data sync console without embedding secrets', () => {
    ...
    expect(html).not.toContain('models.dev') // removed as a data source
    ...
    expect(html).toContain('data-preview="openrouter"')
    expect(html).not.toContain('data-preview="models-dev"') // removed as a data source
    expect(html).not.toContain('data-preview="basellm"')
    expect(html).not.toContain('BaseLLM / NewAPI')
    expect(html).toContain("return'/api/update/'+source+'/'+action")
