...
describe('site renderer', () => {
  it('renders the root page as the model plaza', () => {
    ...
    expect(html).toContain('href="https://raw.githubusercontent.com/imphillip/mddb/main/data/models.json"')
    expect(html).toContain('>models.json</a></div><a class="githubLink"')
  ...
  it('renders a Chinese model plaza with only brand filters and a release-sorted table', () => {
    ...
    expect(html).toContain('.modelsShell{')
    expect(html).toContain('.filterPanel{')
    expect(html).toContain('.mainPanel{')
  ...
  it('renders copy controls beside model name tags so discussions can reference exact ids', () => {
    ...
    expect(detailHtml).toContain('复制模型 tag')
    expect(detailHtml).toContain('viewBox="0 0 24 24"')
    expect(detailHtml).not.toContain('>复制</button>')
  ...
  it('renders complete official pricing components on model detail pages', () => {
    ...
    expect(html).not.toContain('<h2>数据来源</h2>')
  ...
  it('renders model detail as specs first, hides operational sections, and keeps data-policy notes off the page', () => {
    const html = renderModelDetailPage('claude-sonnet-4')

    expect(html).toContain('← 返回模型广场')
    expect(html).toContain('class="btn backToPlaza"')
    expect(html).not.toContain('返回模型列表')
    expect(html).not.toContain('OpenRouter 模型介绍')
    expect(html).not.toContain('mddb.dev 将')
    expect(html).not.toContain('provider 不构成模型变体')
    expect(html).not.toContain('free tier')
    expect(html).not.toContain('raw observation')
    expect(html).not.toContain('<h2>模型身份</h2>')

    const specsIndex = html.indexOf('<h2>模型规格</h2>')
    const pricingIndex = html.indexOf('<h2>价格</h2>')
    expect(specsIndex).toBeGreaterThan(-1)
    expect(pricingIndex).toBeGreaterThan(-1)
    expect(specsIndex).toBeLessThan(pricingIndex)
    expect(html).toContain('<span>规范标签</span>')
    expect(html).toContain('<span>Alias</span>')
    expect(html).toContain('<span>显示名称</span>')
    expect(html).toContain('Snapshot 规格差异')
    expect(html).toContain('价格差异')
    expect(html).not.toContain('<h2>可用来源</h2>')
    expect(html).not.toContain('<h2>数据来源</h2>')
    expect(html).not.toContain('<h2>数据审核</h2>')
    expect(html).not.toContain('<h2>评测</h2>')
    expect(html).not.toContain('<h2>mddb API</h2>')
    expect(html).not.toContain('<aside>')
    expect(html).not.toContain('<span>延迟</span>')
    expect(html).not.toContain('<span>吞吐</span>')
  })

  it('groups real pricing variants and suppresses source observations identical to the main record', () => {
    const html = renderModelDetailPage('claude-sonnet-4')

    expect(html).toContain('class="priceVariantCard"')
    expect(html).toContain('标准价格')
    expect(html).toContain('价格差异')
