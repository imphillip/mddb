...
describe('registry graph adapter', () => {
  ...
  it('renders the current model plaza and detail UI from the new registry graph', () => {
    ...
    expect(detail).not.toContain('数据来源与源数据')
    expect(node!.raw.model).not.toHaveProperty('mddb_registry')
    expect(node!.raw.model).not.toHaveProperty('prices')
    expect(node!.raw.model).toHaveProperty('offers')
    expect(((node!.raw.model as { offers?: Array<{ prices?: unknown[] }> }).offers ?? []).some((offer) => Array.isArray(offer.prices) && offer.prices.length > 0)).toBe(true)
    expect(detail).not.toContain('&quot;mddb_registry&quot;')
    expect(detail).toContain('&quot;offers&quot;: [')
    ...
    // v2: canonical model pages render their embedded offers, not the legacy provider-deployment fallback card.
    expect(xiaomiSourceDetail).toContain('<h3>OpenRouter</h3>')
    expect(xiaomiSourceDetail).not.toContain('canonical author 暂无自有报价')
    expect(xiaomiSourceDetail).not.toContain('provider 报价')
    ...
    // v2: shows the model's embedded offer card, not the legacy Amazon provider-deployment fallback.
    expect(palmyraSourceDetail).toContain('<h3>OpenRouter</h3>')
    expect(palmyraSourceDetail).toContain('Input / prompt')
