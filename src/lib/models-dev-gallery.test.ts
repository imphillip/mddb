...
describe('models.dev-backed rendering', () => {
  ...
  it('renders models.dev model detail pages and uses dash for missing release dates', () => {
    ...
    expect(detailHtml).toContain('<span class="providerChip">OpenAI</span>')
    expect(detailHtml).toContain('<span class="providerChip">Azure AI Foundry</span>')
    expect(detailHtml).toContain('GPT-4o')
    expect(detailHtml).toContain('模型身份')
    expect(detailHtml).toContain('可用来源')
    expect(detailHtml).toContain('规格差异')
