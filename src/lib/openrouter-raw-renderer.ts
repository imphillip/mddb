...
function currencyToggle(graph: OpenRouterRawGraph): string {
  ...
  return `<div class="currencyControl"><div class="currencyToggle" data-currency-toggle title="${escapeHtml(title)}"><button type="button" class="active" data-currency="USD">USD</button><button type="button" data-currency="CNY">CNY</button></div><span class="fxNote">1 USD ≈ ${escapeHtml(currency.rate.toFixed(1))} CNY</span></div>`
}
