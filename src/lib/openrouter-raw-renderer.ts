...
function relationChips(graph: OpenRouterRawGraph, outEdges: OpenRouterRawEdge[], inEdges: OpenRouterRawEdge[]): string {
  ...
  const chips = [
    ...anchorEdges.map((edge) => renderRelationChip(graph, edge, edge.type === 'spec_same_as' ? 'same spec' : 'anchor', 'to')),
    ...relatedEdges.map((edge) => renderRelationChip(graph, edge, edge.type, 'from')),
    ...deploymentEdges.map((edge) => renderRelationChip(graph, edge, edge.type === 'deployment_of' ? 'provider' : 'endpoint', edge.type === 'deployment_of' ? 'from' : 'to')),
  ]
