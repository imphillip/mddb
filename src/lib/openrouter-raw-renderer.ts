...
.raw{white-space:pre-wrap;overflow:auto;max-height:680px;background:#0b1020;color:#d8e4ff;border-radius:12px;padding:14px;font-size:12px;line-height:1.45}.badge{display:inline-flex;border:1px solid var(--line);background:var(--soft);border-radius:999px;padding:2px 8px;font-size:12px;margin:2px;color:#555}.badge.api{color:var(--green)}.badge.page_only{color:#b91c1c}.statGrid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:10px;margin:18px 0 24px}.statCard{border:1px solid var(--line);border-radius:14px;background:#fff;padding:14px 16px;box-shadow:var(--shadow)}.statCard span{display:block;color:#777;font-size:12px;margin-bottom:4px}.statCard b{font-size:20px;letter-spacing:-.3px}.rawIntro{max-width:880px;color:#666;line-height:1.7}.filterHint{color:#999;font-size:12px;margin-top:8px}.edge{border:1px solid var(--line);border-radius:12px;padding:12px;margin:10px 0;background:#fff}.two{display:grid;grid-template-columns:1fr 1fr;gap:14px}.section{margin-bottom:18px}.kv{display:grid;grid-template-columns:180px minmax(0,1fr);gap:12px;padding:10px 0;border-bottom:1px solid var(--line2)}.kv span{color:#777}.kv b{font-weight:500;word-break:break-word}.tabs{display:flex;gap:8px;flex-wrap:wrap;margin:18px 0}.tabs a{border:1px solid var(--line);border-radius:999px;padding:8px 12px;background:#fff;color:#333}.modelTable td:nth-child(3){text-align:left}.rawSource{word-break:break-all}.mono,code,pre{font-family:Geist Mono,ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}.statusLine{display:flex;flex-wrap:wrap;gap:4px}.providerSummary{color:#777;font-size:12px;margin-top:4px}.searchBox{width:100%;height:38px;border:1px solid var(--line);border-radius:8px;padding:0 12px;font:14px inherit;margin-bottom:14px}@media(max-width:900px){.statGrid,.two{grid-template-columns:1fr}.kv{grid-template-columns:1fr}.modelTable{min-width:900px}}`

type ActivePage = 'home' | 'models'

export function renderOpenRouterRawHome(graph: OpenRouterRawGraph): string {
  const providerOptions = providerFilterOptions(graph)
  const rows = graph.nodes.map(renderModelRow).join('')
  const body = `<main class="modelsShell"><aside class="filterPanel" aria-label="模型筛选"><div class="filterTitle">筛选</div><input class="searchBox" id="q" placeholder="搜索模型 / provider / author / source"><div class="filterHint">当前是 OpenRouter-first raw graph 投影，但 URL provider 是实际部署方。</div>${renderProviderFilterGroup(providerOptions)}</aside><section class="mainPanel"><div class="plazaHead"><div><h1>模型广场</h1><p class="rawIntro">按 <code>/models/&lt;provider&gt;/&lt;model-id&gt;</code> 展示；<code>provider</code> 是实际部署/服务提供方，不是 data source。OpenRouter 作为 <code>dataSource</code> 与 raw JSON 保留；例如 <code>openai/sora-2-pro</code> → <code>/models/openai/sora-2-pro</code>。</p></div></div><div class="statGrid">${stat('API models', graph.stats.apiModels)}${stat('Sitemap pages', graph.stats.sitemapModelPages)}${stat('Page-only', graph.stats.pageOnlyModels)}${stat('Endpoint rows', graph.stats.endpointRows)}${stat('Nodes', graph.stats.nodes)}${stat('Edges', graph.stats.edges)}</div><div class="tableWrap"><table class="modelTable"><thead><tr><th>模型名称</th><th>Provider</th><th>Author</th><th>上下文</th><th>Endpoints</th><th>状态</th></tr></thead><tbody id="rows">${rows}</tbody></table></div><script>${modelFilterScript()}</script></section></main>`
...
export function renderOpenRouterRawDetail(graph: OpenRouterRawGraph, node: OpenRouterRawNode): string {
  ...
  const body = `<main><section class="detailHero detailHeroCompact"><div class="wrap"><a class="btn backToPlaza" href="/models/">← 返回模型广场</a><div class="eyebrow">${escapeHtml(node.providerName)} / ${renderModelTagCopy(`${node.provider}/${node.modelId}`)}</div><h1>${escapeHtml(node.displayName)}</h1><div class="summaryStrip"><div><span>Provider</span><b>${escapeHtml(node.provider)}</b></div><div><span>Model ID</span><b>${escapeHtml(node.modelId)}</b></div><div><span>上下文</span><b>${escapeHtml(node.derived.endpointContextLengths.map(String).join(', ') || '—')}</b></div><div><span>Endpoints</span><b>${node.derived.endpointCount}</b></div></div></div></section><div class="wrap detailSingle databaseDetail"><article><nav class="toc" aria-label="模型页面章节"><a href="#summary">模型规格</a><a href="#graph">Graph</a><a href="#endpoints">Endpoints</a><a href="#raw-model">Raw model</a><a href="#raw-page">Raw page</a><a href="#raw-all">Raw all</a></nav><section id="summary" class="panel"><h2>模型规格</h2><div class="meta metaWide">${kv('Provider', `${escapeHtml(node.providerName)} <code>${escapeHtml(node.provider)}</code>`)}${kv('Provider model-id', renderModelTagCopy(node.modelId))}${kv('Data source', node.dataSource)}${kv('Source ID', renderModelTagCopy(node.sourceId))}${kv('Source URL', `<a href="${escapeHtml(node.sourceUrl)}">${escapeHtml(node.sourceUrl)}</a>`)}${kv('Author', node.derived.author ?? '—')}${kv('Canonical slug', node.derived.canonicalSlug ?? '—')}${kv('Input modalities', node.derived.inputModalities.join(' · ') || '—')}${kv('Output modalities', node.derived.outputModalities.join(' · ') || '—')}${kv('Endpoint providers', node.derived.endpointProviders.join(' · ') || '—')}${kv('Pricing keys', node.derived.pricingKeys.join(' · ') || '—')}</div></section><section id="graph" class="panel subtlePanel"><h2>有向图边</h2><div class="two"><div><h3>Outgoing</h3>${renderEdges(outEdges)}</div><div><h3>Incoming</h3>${renderEdges(inEdges)}</div></div></section><section id="endpoints" class="panel priorityPanel"><h2>Endpoint observations</h2><div class="tableWrap"><table class="modelTable"><thead><tr><th>#</th><th>Provider/tag</th><th>Context</th><th>Raw pricing</th><th>Supported parameters</th></tr></thead><tbody>${endpointRows || '<tr><td colspan="5" class="muted">无 endpoint rows</td></tr>'}</tbody></table></div></section><section id="raw-model" class="panel"><h2>Raw /api/v1/models row</h2>${rawBlock(node.raw.model)}</section><section id="raw-page" class="panel"><h2>Raw model page extraction</h2>${rawBlock(node.raw.page)}</section><section id="raw-all" class="panel"><h2>Raw node</h2>${rawBlock(node)}</section></article></div></main>`
  return page(`${node.displayName} · mddb.dev`, body, 'models')
}

function renderModelRow(node: OpenRouterRawNode): string {
  const modalities = `${node.derived.inputModalities.join(' · ') || '—'} → ${node.derived.outputModalities.join(' · ') || '—'}`
  return `<tr data-model-row data-model-provider="${escapeHtml(node.provider)}" data-model-name="${escapeHtml(`${node.displayName} ${node.provider} ${node.modelId} ${node.sourceId} ${node.derived.author ?? ''}`.toLowerCase())}"><td><div class="modelName">${renderLogoIcon(undefined, `${node.providerName} logo`, node.providerName.slice(0, 1), 'modelIcon')}<div><a class="modelLink" href="${escapeHtml(node.route)}/">${escapeHtml(node.displayName)}</a><div class="modelSub">${renderModelTagCopy(`${node.provider}/${node.modelId}`)}</div><div class="modelSub rawSource">source: ${escapeHtml(node.sourceId)} · ${escapeHtml(modalities)}</div></div></div></td><td>${escapeHtml(node.providerName)}<div class="providerSummary"><code>${escapeHtml(node.provider)}</code></div></td><td>${escapeHtml(node.derived.author ?? '—')}</td><td class="mono">${escapeHtml(node.derived.endpointContextLengths.map(String).join(', ') || '—')}</td><td class="mono">${node.derived.endpointCount}</td><td><div class="statusLine"><span class="badge ${node.status}">${escapeHtml(node.status)}</span>${node.derived.pageOnlyType ? `<span class="badge">${escapeHtml(node.derived.pageOnlyType)}</span>` : ''}<span class="badge">${escapeHtml(node.dataSource)}</span></div></td></tr>`
}
...
function providerFilterOptions(graph: OpenRouterRawGraph): Array<{ label: string; value: string; count: number }> {
  const counts = new Map<string, { label: string; value: string; count: number }>()
  for (const node of graph.nodes) {
    const current = counts.get(node.provider)
    if (current === undefined) counts.set(node.provider, { label: node.providerName, value: node.provider, count: 1 })
    else current.count += 1
  }
  return Array.from(counts.values()).sort((a, b) => b.count - a.count || a.label.localeCompare(b.label)).slice(0, 40)
}

function renderProviderFilterGroup(options: Array<{ label: string; value: string; count: number }>): string {
  return `<div class="filterGroup"><div class="filterHead"><span>Provider</span><span>⌄</span></div>${options.map((option) => renderFilterOption('provider', option)).join('')}</div>`
}

function renderFilterOption(group: string, option: { label: string; value: string; count: number }): string {
  return `<label class="filterOption"><input type="checkbox" data-filter-group="${escapeHtml(group)}" data-filter-value="${escapeHtml(option.value)}">${renderLogoIcon(undefined, `${option.label} logo`, option.label.slice(0, 1), 'filterLogo')}<span>${escapeHtml(option.label)}</span><small>${option.count}</small></label>`
}
...
function applyModelFilters(){
  const providers=selected('provider');
  const query=(q&&q.value||'').toLowerCase();
  modelRows.forEach(row=>{
    const providerOk=providers.length===0||providers.includes(row.dataset.modelProvider||'');
    const queryOk=!query||(row.dataset.modelName||row.innerText||'').toLowerCase().includes(query);
    row.hidden=!(providerOk&&queryOk);
  });
}
