...
function renderAuthorFilterGroup(options: Array<{ label: string; value: string; count: number }>): string {
  return `<div class="filterGroup"><div class="filterHead"><span>Author</span><span>⌄</span></div>${options.map((option) => renderFilterOption('author', option)).join('')}</div>`
}
...
function applyModelFilters(){
  const authors=selected('author');
  const query=(q&&q.value||'').toLowerCase();
  modelRows.forEach(row=>{
    const authorOk=authors.length===0||authors.includes(row.dataset.modelAuthor||'');
    ...
    row.hidden=!(authorOk&&queryOk&&visibilityOk);
  });
}
