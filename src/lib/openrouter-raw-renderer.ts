...
function renderRelatedModels(graph: OpenRouterRawGraph, node: OpenRouterRawNode, outEdges: OpenRouterRawEdge[], inEdges: OpenRouterRawEdge[]): string {
  ...
  const deploymentEdges = inEdges.filter((edge) => ['deployment_of', 'has_endpoint'].includes(edge.type))
  const specSameEdges = outEdges.filter((edge) => edge.type === 'spec_same_as' && edge.from !== edge.to)
  const sections = [
    anchorEdges.length > 0 ? `<div><h3>建议查看 anchor / canonical deployment</h3><div class="relationChips">${anchorEdges.map((edge) => renderRelationChip(graph, edge, edge.type === 'spec_same_as' ? 'same spec' : 'anchor', 'to')).join('')}</div></div>` : '',
    relatedEdges.length > 0 ? `<div><h3>Alias / Snapshot</h3><div class="relationChips">${relatedEdges.map((edge) => renderRelationChip(graph, edge, edge.type, 'from')).join('')}</div></div>` : '',
    deploymentEdges.length > 0 ? `<div><h3>部署 / Providers</h3><div class="relationChips">${deploymentEdges.map((edge) => renderRelationChip(graph, edge, edge.type === 'deployment_of' ? 'provider' : 'endpoint', edge.type === 'deployment_of' ? 'from' : 'to')).join('')}</div></div>` : '',
    specSameEdges.length > 0 ? `<div><h3>相同规格/价格引用</h3><div class="relationChips">${specSameEdges.map((edge) => renderRelationChip(graph, edge, 'same spec', 'to')).join('')}</div></div>` : '',
  ].filter(Boolean).join('')
  return `<section id="relations" class="panel priorityPanel"><h2>关联模型</h2>${sections || '<p class="muted">暂无已确认关联；此节点可能是 anchor 或 unresolved source model。</p>'}</section>`
}
...
function renderSpecSection(node: OpenRouterRawNode): string {
  return `<section id="spec" class="panel"><h2>规格</h2><div class="meta metaWide">${kv('Model ID', renderModelTagCopy(node.modelId))}${kv('Author', node.derived.author ?? '—')}${kv('Input modalities', node.derived.inputModalities.join(' · ') || '—')}${kv('Output modalities', node.derived.outputModalities.join(' · ') || '—')}${kv('Context length', modelContextLength(node))}${kv('Max output tokens', modelMaxOutputTokens(node))}${kv('Tokenizer', rawModelField(node, 'architecture.tokenizer'))}${kv('Supported parameters', rawModelArray(node, 'supported_parameters').join(' · ') || '—')}${kv('Knowledge cutoff', rawModelField(node, 'knowledge_cutoff'))}${kv('Released', modelReleasedDate(node))}</div></section>`
}
...
function renderPricingSection(node: OpenRouterRawNode, endpointRows: string): string {
  ...
  return `<section id="pricing" class="panel"><h2>价格</h2>${sourcePricing}${endpointPricing}<h3>Endpoint raw observations</h3><div class="tableWrap"><table class="modelTable"><thead><tr><th>#</th><th>Provider/tag</th><th>Context</th><th>Raw pricing</th><th>Supported parameters</th></tr></thead><tbody>${endpointRows || '<tr><td colspan="5" class="muted">无 endpoint rows；如本节点为 alias/snapshot/deployment，请先看上方关联模型跳转到 anchor。</td></tr>'}</tbody></table></div></section>`
}

function endpointPricingCards(node: OpenRouterRawNode): string {
  const endpoints = endpointList(node)
  if (endpoints.length === 0) return ''
  return `<h3>Provider pricing</h3><div class="priceVariantGrid">${endpoints.map(renderEndpointPricingCard).join('')}</div>`
}

function renderEndpointPricingCard(endpoint: Record<string, unknown>): string {
  const provider = escapeHtml(String(endpoint.provider_name ?? endpoint.tag ?? 'Provider'))
  const pricing = isRecord(endpoint.pricing) ? endpoint.pricing : {}
  return `<div class="priceVariantCard"><strong>${provider}</strong><div class="meta metaWide">${kv('Currency', 'USD')}${kv('Context length', escapeHtml(String(endpoint.context_length ?? '—')))}${kv('Max output tokens', escapeHtml(String(endpoint.max_completion_tokens ?? '—')))}${kv('Input / prompt', formatPrice(pricing.prompt, 'per token'))}${kv('Output / completion', formatPrice(pricing.completion, 'per token'))}${kv('Cache read', formatPrice(pricing.input_cache_read, 'per token'))}${kv('Web search', formatPrice(pricing.web_search, 'per request'))}${kv('Discount', escapeHtml(String(pricing.discount ?? '—')))}</div></div>`
}

function formatPrice(value: unknown, unit: string): string {
  if (value === null || value === undefined || value === '') return '—'
  return `<code>${escapeHtml(String(value))}</code> <span class="muted">${escapeHtml(unit)}</span>`
}
...
function nav(activePage: ActivePage): string {
  ...
  return `<header class="topbar"><nav class="nav"><a class="brandmark" href="/models/">${databaseLogo()}<span>mddb.dev</span><span class="brandZh">大模型数据库</span></a>${search}<div class="navlinks"><span class="disabled" aria-disabled="true">模型动态</span><a${modelsClass} href="/models/">模型广场</a></div><a class="githubLink" href="https://github.com/imphillip/mddb" target="_blank" rel="noopener noreferrer" aria-label="GitHub 仓库">${githubLogo()}</a></nav></header>`
}
