...
export function renderOpenRouterRawHome(graph: OpenRouterRawGraph): string {
  const rows = graph.nodes.map((node) => `<tr><td><a href="${escapeHtml(node.route)}/"><strong>${escapeHtml(node.displayName)}</strong></a><br><code>${escapeHtml(node.sourceId)}</code></td><td><span class="badge ${node.status}">${escapeHtml(node.status)}</span>${node.derived.pageOnlyType ? `<span class="badge">${escapeHtml(node.derived.pageOnlyType)}</span>` : ''}</td><td>${escapeHtml(node.derived.author ?? '—')}</td><td>${escapeHtml(node.derived.inputModalities.join(', ') || '—')} → ${escapeHtml(node.derived.outputModalities.join(', ') || '—')}</td><td>${node.derived.endpointCount}</td><td>${escapeHtml(node.derived.endpointContextLengths.map(String).join(', ') || '—')}</td></tr>`).join('')
  return page('OpenRouter raw graph · mddb.dev', `<main class="wrap"><section class="hero"><h1>OpenRouter Raw Provider Graph</h1><p class="muted">按 <code>/models/openrouter/&lt;model-id&gt;</code> 保存。这里尽量原样展示 OpenRouter 返回的 key/value；派生字段只用于索引、有向图和调阅。</p><div class="grid">${stat('API models', graph.stats.apiModels)}${stat('Sitemap pages', graph.stats.sitemapModelPages)}${stat('Page-only', graph.stats.pageOnlyModels)}${stat('Endpoint rows', graph.stats.endpointRows)}${stat('Nodes', graph.stats.nodes)}${stat('Edges', graph.stats.edges)}</div></section><section class="panel"><input class="search" id="q" placeholder="过滤 model id / display name / author"><table class="table"><thead><tr><th>模型</th><th>状态</th><th>Author</th><th>Modalities</th><th>Endpoints</th><th>Context lengths</th></tr></thead><tbody id="rows">${rows}</tbody></table></section><script>${filterScript()}</script></main>`)
}
