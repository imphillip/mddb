...
function nav(activePage: ActivePage, navExtra = ''): string {
  ...
  return `<header class="topbar"><nav class="nav"><a class="brandmark" href="/models/">${databaseLogo()}<span>mddb.dev</span><span class="brandZh">大模型数据库</span></a>${search}<div class="navlinks"><span class="disabled" aria-disabled="true">模型动态</span><a${modelsClass} href="/models/">模型广场</a></div>${navExtra}<a class="githubLink" href="https://github.com/imphillip/mddb" target="_blank" rel="noopener noreferrer" aria-label="GitHub 仓库">${githubLogo()}</a></nav></header>`
}

function currencyToggle(graph: OpenRouterRawGraph): string {
  const currency = graph.currency
  if (!currency) return ''
  const title = `1 USD ≈ ${currency.rate.toFixed(1)} CNY · ${currency.source}`
  return `<div class="currencyControl"><div class="currencyToggle" data-currency-toggle title="${escapeHtml(title)}"><button type="button" class="active" data-currency="USD">USD</button><button type="button" data-currency="CNY">CNY</button><span class="fxInline">1 USD ≈ ${escapeHtml(formatDisplayNumber(currency.rate))} CNY</span></div></div>`
}
