...
describe('site renderer', () => {
  ...
  it('renders database-first model detail sections with lightweight provider availability', () => {
    ...
    expect(html).toContain('大模型数据库')
    expect(html).toContain('模型身份')
    expect(html).toContain('官方价格')
    expect(html).toContain('模型规格')
    expect(html).toContain('规格差异')
    expect(html).toContain('可用来源')
    expect(html).toContain('数据来源')
    expect(html).toContain('数据审核')
    expect(html).toContain('mddb API')
    expect(html).toContain('输入价格')
    expect(html).toContain('输出价格')
    expect(html).toContain('复制模型 tag')
    expect(html).toContain('只列出 provider 可用性；除非上下文、价格或能力不同，否则 provider 不构成模型变体。')
