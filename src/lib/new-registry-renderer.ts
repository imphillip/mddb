...
export function renderNewModelsHome(registry: NewRegistry): string {
  ...
  const rows = registry.offers
    ...
    .map(renderOfferRow)
    .join('')
  const body = `<main class="modelsShell"><aside class="filterPanel" aria-label="新模型筛选"><div class="filterGroup"><div class="filterHead"><span>Provider</span><span>⌄</span></div>${renderProviderFilterOption({ label: '全部', value: 'all', count: registry.offers.length }, true)}${providerOptions.slice(0, 20).map((option) => renderProviderFilterOption(option)).join('')}</div><div class="filterGroup"><h3>实验数据</h3><p class="filterHint">LiteLLM-first，OpenRouter 补充。用于观察新 schema，不代表正式目录。</p><p class="filterHint">${registry.stats.providerCount} providers · ${registry.stats.offerCount} offers · ${registry.stats.pricedOfferCount} priced</p></div></aside><section class="mainPanel"><div class="plazaHead"><div><h1>New Models 实验目录</h1><p class="rawIntro">基于 .internal/next-registry 的 provider/offer 数据预览。</p></div></div><div class="tableWrap"><table class="modelTable"><thead><tr><th>模型</th><th>上下文</th><th>输入<br><small>/M tokens</small></th><th>输出<br><small>/M tokens</small></th><th>读取<br><small>/M tokens</small></th><th>Endpoint</th></tr></thead><tbody id="rows">${rows}</tbody></table></div><script>${newModelsFilterScript()}</script></section></main>`
...
function page(title: string, body: string, activePage: 'models'): string {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><link rel="stylesheet" href="/assets/new-models.css"></head><body>${nav(activePage)}${body}<script>${copyModelTagScript()}</script>${footer()}</body></html>`
}
