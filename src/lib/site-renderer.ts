...
function renderProviders(model: ModelDetail): string {
  ...
  return Array.from(providers.values()).map((p) => `<div class="providerRow"><div class="rowTop"><strong>${renderLogoIcon(p.logoUrl, `${p.name} logo`, p.name.slice(0, 1), 'providerIcon')}${escapeHtml(p.name)}</strong><span class="pill">可用率 ${escapeHtml(p.uptime)}</span></div><div class="meta"><div class="metabox"><span>区域</span><b>${escapeHtml(p.region)}</b></div><div class="metabox"><span>延迟</span><b>${escapeHtml(p.latency)}</b></div><div class="metabox"><span>吞吐</span><b>${escapeHtml(p.throughput)}</b></div></div></div>`).join('')
}
