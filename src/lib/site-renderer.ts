...
function renderFilterGroup(title: string, group: string, options: Array<{ label: string; value: string; count: number }>): string {
  const renderedOptions = options
    ...
    .map((option) => `<label class="filterOption"><input type="checkbox" data-filter-group="${escapeHtml(group)}" data-filter-value="${escapeHtml(option.value)}"><span class="check"></span><span>${escapeHtml(option.label)}</span><small>${option.count}</small></label>`)
    .join('')
...
export function renderModelDetailPage(tag: string, details?: ModelDetail[]): string {
  ...
  return page(`${model.name} · mddb.dev`, `<main><section class="detailHero"><div class="wrap"><div class="eyebrow">${escapeHtml(model.brand.name)} / ${renderModelTagCopy(model.tag)}</div><h1>${escapeHtml(model.name)}</h1><p>${escapeHtml(model.longDescription)}</p></div></section><div class="wrap detailGrid"><article><nav class="toc" aria-label="Model page sections"><a href="#overview">Overview</a><a href="#providers">Providers</a><a href="#variants">Variants</a><a href="#pricing">Pricing</a><a href="#benchmarks">Benchmarks</a><a href="#api">API</a></nav><section id="overview" class="panel"><h2>Overview</h2><p class="muted">${escapeHtml(model.description)}</p><div class="meta"><div class="metabox"><span>MODALITIES</span><b>${escapeHtml(joinChinese(model.modalities))}</b></div><div class="metabox"><span>WEEKLY TOKENS</span><b>${escapeHtml(model.weeklyTokens)}</b></div><div class="metabox"><span>RELEASED</span><b>${escapeHtml(formatDate(model.releasedAt))}</b></div></div></section><section id="providers" class="panel"><h2>Providers</h2>${renderProviders(model)}</section><section id="variants" class="panel"><h2>Variants</h2>${model.variants.map(renderVariant).join('')}</section><section id="benchmarks" class="panel"><h2>Benchmarks</h2>${model.benchmarks.map((b) => `<div class="providerRow"><div class="rowTop"><strong>${escapeHtml(b.name)}</strong><span class="pill">${escapeHtml(b.score)}</span></div><p class="muted">${escapeHtml(b.note)}</p></div>`).join('')}</section><section id="api" class="panel"><h2>API</h2><div class="apiBox">GET /models/${escapeHtml(model.tag)}<br>{<br>&nbsp;&nbsp;"modelTag": "${escapeHtml(model.tag)}",<br>&nbsp;&nbsp;"variants": ${model.variants.length}<br>}</div></section></article><aside><div id="pricing" class="panel"><h2>Pricing</h2><div class="priceGrid"><div class="metabox"><span>INPUT PRICE</span><b>${escapeHtml(model.inputPrice)}</b></div><div class="metabox"><span>OUTPUT PRICE</span><b>${escapeHtml(model.outputPrice)}</b></div><div class="metabox"><span>CONTEXT</span><b>${escapeHtml(model.contextWindow)}</b></div><div class="metabox"><span>VARIANTS</span><b>${model.variants.length}</b></div></div></div><div class="panel"><h2>Providers</h2><p class="muted">${escapeHtml(joinChinese(model.providerNames))}</p></div></aside></div></main>`, 'models')
}
