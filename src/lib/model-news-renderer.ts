...
export function renderModelNewsHome(_graph: OpenRouterRawGraph, feed: ModelNewsFeed): string {
  ...
  const body = `<main class="modelsShell newsShell"><aside class="filterPanel newsInfoPanel" aria-label="模型动态说明"><div class="filterGroup"><h3>模型动态</h3><p class="filterHint">来自 AIHOT mode=all，经 mddb.dev provider / model-id 标签过滤。</p></div><div class="filterGroup"><h3>统计</h3><div class="listCount"><b>${items.length}</b> 条已标注动态</div></div></aside><section class="mainPanel newsPanel"><div class="plazaHead newsHead"><div><h1>模型动态</h1><p class="rawIntro">追踪全球 AI 模型发布、更新、评测、使用技巧、用户反馈与应用场景。</p></div></div><div class="newsTimeline">${groups.map(renderDateGroup).join('') || '<p class="muted">暂无已标注模型动态。</p>'}</div></section></main>`
